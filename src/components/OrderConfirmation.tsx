      ...
      {!ready ? null : !order ? (
        ...
        <div className="mx-auto max-w-[560px]">
          <div className="flex flex-col items-center text-center">
            ...
            <h1 className="heading-track mt-5 text-[28px] font-medium text-ink md:text-[34px]">Thank you for your order!</h1>
            <p className="mt-2 text-[16px] text-ink">Order {order.number}</p>
            <p className="mt-1 text-[14px] text-body">A confirmation was sent to {order.email}.</p>
          </div>
          ...
          <div className="mt-10 rounded-md border border-line bg-white p-6">
            ...
            <div className="mt-6 space-y-2 border-t border-line pt-5 text-[14px]">
              <div className="flex items-center justify-between text-ink"><span>Subtotal</span><span>{formatPrice(order.subtotal, order.currency)}</span></div>
              <div className="flex items-center justify-between text-ink"><span>Shipping</span><span>{order.shipping === 0 ? "FREE" : formatPrice(order.shipping, order.currency)}</span></div>
              {order.discount > 0 && <div className="flex items-center justify-between text-mauve"><span>Discount</span><span>−{formatPrice(order.discount, order.currency)}</span></div>}
              <div className="flex items-center justify-between text-ink"><span>Tax</span><span>{formatPrice(order.tax, order.currency)}</span></div>
              <div className="flex items-center justify-between border-t border-line pt-3 text-[16px] font-medium text-ink"><span>Total</span><span>{formatPrice(order.total, order.currency)}</span></div>
            </div>
          </div>
